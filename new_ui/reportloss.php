...
if( (!isset($areaaffectedError)) AND (!isset($affectedstateError)) AND (!isset($affecteddistrictError))  AND (!isset($affectedtownError)) AND (!isset($affectedtownError)) AND (!isset($affectedpinError)) AND (!isset($firstnameError)) AND (!isset($lastnameError)) AND (!isset($usernameError)) AND (!isset($passwordError)) )
{
   ...
   $sql = "INSERT INTO report (id,firstname,lastname,username,password,croptype, areaaffected,affectedstate,affecteddistrict, affectedtown, affectedpin,status) VALUES ( '" . $id . "','" . $firstname . "', '" . $lastname . "', '" . $username . "', '" . $password . "','" . $croptype . "', '" . $areaaffected . "','" . $affectedstate . "','" . $affecteddistrict . "', '" . $affectedtown . "', '" . $affectedpin . "','" . $status . "');";
...
<form action="reportloss.php" method="post" role="form" class="contactForm" style="margin-left: 300px">
  ...
  <div class="span4 form-group">
      <input type="number" name="id" class="form-control" id="" placeholder="ID" value="<?php echo $id?>" />
